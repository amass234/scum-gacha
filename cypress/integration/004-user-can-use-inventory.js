...
describe("User can use inventory", () => {
  ...
  it("Validates the inventory", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      cy.get(`.banner-button.${banner}`).click();
      cy.get(".wish-button")
      ...
      cy.get(
        ...
        .then(() => {
          ...
          cy.get("select#view").select("Icons");
          cy.get("select#showOnly").select("Characters");
          cy.get("select#showOnly").select("Weapons");
          cy.get("select#showOnly").select("5 Stars");
          cy.get("select#showOnly").select("4 Stars");
          cy.get("select#showOnly").select("3 Stars");
          cy.get("select#showOnly").select("All");
          cy.get("select#orderBy").select("Name");
          cy.get("select#orderBy").select("Quantity");
          cy.get("select#orderBy").select("Rating");

          cy.get("select#view").select("List");
          cy.get("select#showOnly").select("Characters");
          cy.get("select#showOnly").select("Weapons");
          cy.get("select#showOnly").select("5 Stars");
          cy.get("select#showOnly").select("4 Stars");
  ...
  it("Preserves Inventory in Local Storage", () => {
    ...
    cy.get("h4").contains("No Items :(");
    cy.get('img[alt="Sad paimon"]').should("be.visible");
  });
});
