import React from "react";
import { Col, Row } from "reactstrap";
import ReactStars from "react-rating-stars-component";
const characterImages = require.context("../assets/images/characters");
const weaponImages = require.context("../assets/images/weapons");
const characterIcons = require.context(
  "../assets/images/details/character-icons"
);
export default function WishItemSingle(props) {
  const { isNewItem } = props;
  const { src, name, rating, type } = props.item;
  const isCharacter = type === "character";
  const backgroundImage = `url('${
    isCharacter
      ? characterImages("./" + src).default
      : weaponImages("./" + src).default
  }')`;
  return (
    <Col xs="12" className="h-100 p-4">
      <Row className="wish-item-single-container h-100 p-2">
        {isNewItem && <span className="new-badge">New</span>}
        <Col
          className="h-100 d-flex flex-column justify-content-center align-items-center wish-item-single-content"
          style={{
            background:
              isCharacter &&
              `url('${
                characterIcons(`./${name}-Icon.png`).default
              }') left / 20% no-repeat`,
          }}
          sm="12"
          md="3"
        >
          <div className="text-center text-wrap pb-1">{name}</div>
          <ReactStars
            count={rating}
            size={24}
            edit={false}
            color="#ffd700"
            classNames={"justify-content-center"}
          />
        </Col>
        <Col className="wish-item-single">
          <img
            width="190"
            src={`${
              isCharacter
                ? characterImages("./" + src).default
                : weaponImages("./" + src).default
            }`}
          />
        </Col>
      </Row>
    </Col>
  );
}
